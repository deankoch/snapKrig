...
#' locations, and Z is the spatial process value. `blitzKrig` assumes that Z is second-order
#' stationary, which means that `v` only depends on the relative displacement s1-s2.
#' `v` in this case is equal to twice the covariance function. `bk_vario_fun` computes
...
#' vario = bk_vario_fun(pars, d=d_equal)
...
#' vario_lims = bk_vario_fun(pars, d=d_test)
#' lines(d_test, vario_lims[,1])
#' lines(d_test, vario_lims[,2])
#'
bk_vario_fun = function(pars, d=NULL)
{
  # 1d vector case
  if( !is.list(d) )
  {
    ...
    cov_y = bk_vario_fun( pars, list(y=d, x=d0) )
    cov_x = bk_vario_fun( pars, list(y=d0, x=d) )
    cov_yx = bk_vario_fun( pars, list(y=d/sqrt(2), x=d/sqrt(2)))
  ...
  corrvals = do.call('*', Map(function(p, dyx) bk_corr(p, dyx), p=pars[nm_yx], dyx=d[nm_yx]))
...
#' idx_sample = bk_sample_pt(g=gdim)
#' bk_plot(seq(ng) %in% idx_sample, gdim)
#'
#' # reduce or increase number of center points from default 100
#' idx_sample = bk_sample_pt(gdim, n=10)
#' bk_plot(seq(ng) %in% idx_sample, gdim)
#'
#' # sampled from Moore neighbourhoods of radius 6
#' n = 10
#' idx_sample = bk_sample_pt(gdim, n=n, lag_max=6L)
#' bk_plot(seq(ng) %in% unlist(idx_sample), gdim, col_grid='white')
#'
#' # plot each list element a different color
#' group_sample = rep(0L, prod(gdim))
#' for(i in seq(1 + n)[-1]) group_sample[ idx_sample[[i]] ] = i-1L
#' bk_plot(group_sample, gdim, breaks=c('not sampled', seq(n)), zlab='sub-grid')
#'
...
#' idx_sample = bk_sample_pt(gdim, n=n, interval=interval, lag_max=lag_max)
#' idx_overlap = rowSums( sapply(idx_sample[-1], function(i) seq(ng) %in% i) )
#' bk_plot(as.integer(idx_overlap), gdim, zlab='times sampled')
#'
#' # plot each list element a different color
#' group_sample = rep(0L, prod(gdim))
#' for(i in seq(1 + interval^2)[-1]) group_sample[ idx_sample[[i]] ] = i-1L
#' bk_plot(group_sample, gdim, breaks=c('not sampled', seq(interval^2)), zlab='sub-grid')
#'
#' # compare with over=TRUE (usually results in overlap - try running a few times)
#' idx_sample_compare = bk_sample_pt(gdim, n=n, interval=interval, lag_max=lag_max, over=TRUE)
...
#' idx_sample = bk_sample_pt(g, n=10, lag_max=6L, interval=2)
...
bk_sample_pt = function(g, n=1e2, lag_max=0, interval=1L, over=FALSE)
{
  ...
  if(lag_max == 0) { idx_list = list(idx_center_all) } else {
    ...
    if(!over)
    {
      ...
      idx_disjoint = lapply(ij_all, function(ij) bk_sub_idx(gdim_inner, unname(ij), idx=TRUE))
    ...
    idx_list = lapply(idx_center_all, function(idx) {
      ...
      ij_center = lapply(bk_vec2mat(idx, gdim, out='list'), function(idx) idx + box_offset)

      # find vectorized index corresponding to this sample
      bk_sub_idx(gdim, ij_center, idx=TRUE)
    })
...
#' g_obs[['gval']] = bk_sim(g=g_obs, pars)
...
#' bk_plot_semi(bk_sample_vg(g_obs, 1e2), pars)
#' bk_plot_semi(bk_sample_vg(g_obs, 1e3), pars)
#'
#' # use a set of specific points
#' n_sp = 10
#' ( n_sp^2 - n_sp ) / 2 # the number of point pairs
#' vg = bk_sample_vg(g_obs, idx=sample.int(n, n_sp))
#' bk_plot_semi(vg, pars)
#'
#' # repeat with all point pairs sampled (not recommended for big data sets)
#' vg = bk_sample_vg(g_obs, n_pp=Inf)
#' bk_plot_semi(vg, pars)
...
bk_sample_vg = function(g, n_pp=1e4, idx=NULL, n_bin=25, n_layer_max=NA, quiet=FALSE)
{
  ...
  if( is.null(idx) ) idx = bk_sample_pt(g, n_obs)
  ...
  idx_pp = lapply(ij_pp, function(ij) bk_mat2vec(ij, gdim))
...
#'
#' The function is called by `bk_sample_vg` and `bk_plot_semi` (when column `bin` is
...
#' bk_add_bins(distance_df)
#'
#' # specify fewer bins and set up quantiles explicitly
#' bk_add_bins(distance_df, n_bin = 5) # same as ...
#' bk_add_bins(distance_df, n_bin = 5, probs=seq(0, 1, length.out=6))
#'
#' # break range of distances into evenly spaced bins (of varying sample sizes)
#' bk_add_bins(distance_df, n_bin = 5, probs=NULL)
#'
bk_add_bins = function(vg, n_bin=25, probs=NULL)
{
