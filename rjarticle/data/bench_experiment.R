...
#' ---
#'
#' NOTE: If you're running this code, make sure `dir_bench_results` (declared below) is located on a
#' drive with at least 4GB of free space. By default it is set to the sub-directory "bench_output".
#'
#' ## Overview
#'
#' This script generates the benchmarking results reported in the paper. It records evaluation time
#' for ordinary kriging on several datasets, comparing four different R packages.
#'
#' The entire script takes about 6 hours to finish with default parameters and it writes 3.8GB
#' of output files to `dir_bench_results`. This can all be deleted afterwards if you are only
#' interested in the summary results, which are saved to a small CSV file "bench_results.csv"
#' in `dir_storage`.
#'
#' The script also writes the files "treed.tif" and "treed_dem.tif" to `dir_storage` if they
#' don't exist already. These data can be downloaded using the `rasterbc` package.
#'
#' ## Dependencies
#'
...
#' packages. To keep this document tidy, I defined these helper functions in two separate
...
path_deps = file.path(dir_storage, c('data_helpers.R', 'bench_experiment_helpers.R'))
...
#' location with at least 3.8GB of free storage space. This does not necessarily need
#' to be a sub-directory of `dir_project`.
#'
...
dir_bench_results = file.path(dir_storage, 'bench_output')
...
# factors by which to up-scale the treed raster (see Input Files section in the paper)
up_test = 2^(7:0)
...
bench_exists = dir.exists(dir_bench_results)
if(!bench_exists) dir.create(dir_bench_results)
...
treed = get_mpb(out='treed', rasterbc_storage_path=dir_bench_results)
treed_dem = get_mpb(out='dem', rasterbc_storage_path=dir_bench_results)

# save the treed and dem rasters as geotiff if they are not found in expected location
if(!file.exists(path_treed)) terra::writeRaster(treed, path_treed)
if(!file.exists(path_treed_dem)) terra::writeRaster(treed_dem, path_treed_dem)
...
path_fit_results = file.path(dir_bench_results, paste0(names(eg_pts), '_fit_results.rds'))
