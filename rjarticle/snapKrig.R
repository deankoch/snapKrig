...
## ----meuse-ok-pred-png, results='hide', fig.show='hold', out.width='50%', fig.dim=c(5,5), fig.cap='Ordinary kriging prediction and variance heatmaps generated by snapKrig for the Meuse example. Predictions are generated for the entire grid, but they are masked in this plot to show detail in areas nearest the observed points.', fig.alt='Two heatmap images are shown, both masked to the same region, covering the convex hull of the observed points, and displaying a range of smoothly varying colors'----
...
# second row
# 
# # prepare sample variograms and find common distance limits
# vg_list = lapply(pars_list, \(p) sk_sample_vg(g-sk_GLS(g, p, X=X), quiet=TRUE) )
# dmax = 2 * max( sapply(pars_list, \(p) p$y$kp[1]) )
# 
# plot_result = Map(\(v, p) {
#   
#   sk_plot_semi(v, p, 
...
## ----meuse-alt-pred-png, results='hide', fig.show='hold', out.width='33%', fig.dim=c(5,6), fig.pos='!bht', fig.cap='Universal kriging predictions for the Meuse example using three anisotropic Kronecker covariance models, based on the Gaussian, exponential, and spherical correlograms.', fig.alt=''----
