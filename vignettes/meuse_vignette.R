...
#'
#' All plots in this document except the first are generated by `bk_plot`, which uses base graphics
...
# plot the grid only
g = bk_grid(modifyList(g_meuse, list(gval=NULL)))
bk_plot(modifyList(g_meuse, list(gval=NULL)), col_grid=NA)
...
fit_result_OK = bk_fit(g_obs=g_meuse, quiet=TRUE)
#fit_result_OK = bk_fit(g_obs=g_meuse, pars='mat', quiet=TRUE)
#vg_detrend = bk_sample_vg(g_meuse)
#bk_plot_semi(vg_detrend, fit_result_OK$pars)
...
fit_result_UK = bk_fit(g_obs=g_meuse, X=X, quiet=TRUE)
...
#'
#' These fitted kernels can be visualized using `bk_plot_pars`. This plots the
...
# vg_detrend = bk_sample_vg(g2)
# bk_plot_semi(vg_detrend, pars_UK, fun='classical')
# bk_plot_semi(vg_detrend, pars_UK, fun='root_median')



#'
#' Once you have a set of covariance parameters, interpolation becomes very easy
#' with `bk_cmean`. This computes the kriging predictor, an interpolator with nice
...
#'
#' Kriging variance is also computed with `bk_cmean`. This is much slower
...
#' detail at how `bk_cmean` actually works, and how users can modify the workflow to
