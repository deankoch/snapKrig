---
title: "snapKrig Meuse vignette"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{snapKrig Meuse vignette}
...
# copy default graphical parameters
...
## A brief introduction to snapKrig

snapKrig is for modeling spatial processes in 2-dimensions and working with associated
grid data. There is an emphasis on computationally fast methods for kriging
but the package offers much more, including its own (S3) grid object class, via the
grid constructor function `sk`. 

### sk grid objects

Start with a simple example, the identity matrix. Pass this (or any other) matrix to `sk` to get a grid object
...
An sk object stores the matrix values (if any) and the dimensions, and it assigns default x and y
...
snapKrig has many other useful methods implemented for the sk class, including operators like `+` and `==`
...
helpful for getting some intuition about their structure. For example the next plot shows a covariance
...
Some symmetries are immediately obvious: the banding; the blocks; the Toeplitz structures - both
...
snapKrag is unusually fast at generating spatially auto-correlated data like this and it supports
a number of different covariance models. In simple terms, this changes the general
appearance, size, and connectivity of the random blobs in the image above. See `?sk_corr` for
more on these models.

### Covariance plots

Use `sk_plot_pars` to visualize a covariance parameter set. This shows the footprint of
...
However, snapKrig functionality is focused on spatial models and kriging. Outside of that context we
recommend managing raster data with other packages (`terra` in particular). `sk` will accept single
and multi-layer rasters from the `terra` and `raster` packages, reshaping them as sk grid objects,
and any sk grid can be converted to a SpatRaster or Rasterlayer using `sk_export`.
...
The results in the above plot look impressive - the predictions are almost identical to the earlier
...
To illustrate, suppose the two range parameters in the model are the only unknowns. We could
make a list of plausible values for the parameters and check the likelihood for each one.
...
To organize our results, we make a grid out of the test values (similar to
...
# identify the maximum likelihood test point
...
However the grid search approach used here is not a very efficient way of doing MLE and there are many good
alternatives (just have a look at the CRAN's Optimization Task View). snapKrig implements MLE for
covariance models in `sk_fit` using `stats::optim`. The next section demonstrates it on a real life
dataset.
...
These points are the subject of a
[kriging vignette for gstat](https://cran.r-project.org/web/packages/gstat/vignettes/gstat.pdf) -
which we loosely follow in this vignette -
and they are lazy-loaded with the `sp` package. For general purpose work on geo-referenced
vector data in R, we recommend using the more up-to-date `sf` package. Both are loaded below:   
...
with `sk` object plots, one can overlay additional spatial vector layers using the `reset` and `add`
arguments.

```{r meuse_snapped_plot, out.width='50%', fig.dim=c(5,5), fig.align='center'}
# plot gridded version using the snapKrig package
plot(g, reset=FALSE, zlab='log(ppb)', main='snapped Meuse log[zinc] data')
plot(meuse_sf[['river_poly']], col='lightblue', border=NA, add=TRUE)
...
of covariates (*ie* the trend) and the other is the random spatial component, which has
been interpolated from the observed points.
...
wish to disentagle them (for example if the trend is known *a priori*), in which case the response 
data (`g`) should be de-trended, and `X` should be set to 0 (not `NA`) in the `sk_fit` and `sk_cmean`
calls. This is called *simple kriging*
...
compute it directly, rather than having to approximate (through bootstrap, CV, MCMC etc).
...
uncertainty. This reflects a (spatially constant) measurement error that is represented in the model by the nugget
effect (find it in list element 'eps' of the parameter list returned by `sk_fit`).
...
the nugget to zero (producing an exact interpolator), this can have unpredictable effects resulting
from numerical precision limits on computers.
...
we are probably after the untransformed values. Taking `exp(g_uk)`, while intuitive, would introduce a negative bias.
...
In short, to get zinc concentration predictions on the original scale, we need a bias adjustment. Cressie
supplies one for logarithm in his book, and we use a simplified version of that below (add half the variance
before exponentiating). The plot below shows the result, with the original observed point data overlaid.
...
sk_plot(g_uk_orig, zlab='zinc (ppm)', main='[zinc] predictions and observations', cex=1.5, zlim=zlim, reset=FALSE)
plot(meuse_sf[['river_line']], add=TRUE)
