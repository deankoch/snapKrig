...
This vignette shows how to use `blitzKrig` to interpolate the Meuse river
dataset (part of the `sp` package) onto a high resolution grid:

-   `bk_grid` defines the grid
-   `bk_snap` snaps point data to the grid
-   `bk_fit` fits a covariance model to the data
-   `bk_cmean` computes the kriging predictor and variance

All plots in this document except the first are generated by
`bk_plot`, which uses base graphics to display rasters and matrices.
Try setting `ij=TRUE` in any of the plot calls to see a different
styling used for matrices.

## Getting started

`blitzKrig` has no requirements outside of base packages (`stats`, `utils`,
...
![](https://github.com/deankoch/blitzKrig/blob/main/vignettes/meuse_vignette_files/figure-gfm/meuse_load-1.png)<!-- -->
...
g_meuse = bk_snap(meuse[['soils']]['log_zinc'], g=list(gres=gres))

# plot the grid only
bk_plot(modifyList(g_meuse, list(gval=NA)), col_grid=NA)
```

![](https://github.com/deankoch/blitzKrig/blob/main/vignettes/meuse_vignette_files/figure-gfm/snap_grid-1.png)<!-- -->

``` r
# plot with source points indicated over their snapped grid location
bk_plot(g_meuse, zlab='log(zinc)', reset=FALSE)
plot(sf::st_geometry(meuse[['soils']]), add=TRUE)
```

![](https://github.com/deankoch/blitzKrig/blob/main/vignettes/meuse_vignette_files/figure-gfm/snap_grid-2.png)<!-- -->

Pass this data to bk_fit to do ordinary kriging

``` r
# ordinary kriging: fit isotropic gaussian model by default
fit_result_OK = bk_fit(g_obs=g_meuse, pars='gau', quiet=TRUE)
```

![](https://github.com/deankoch/blitzKrig/blob/main/vignettes/meuse_vignette_files/figure-gfm/ordinary_kriging-1.png)<!-- -->
...
g_meuse_sf = bk_coords(g_meuse, out='sf')
```
...
fit_result_UK = bk_fit(g_obs=g_meuse, pars='gau', X=X, quiet=TRUE)
```

![](https://github.com/deankoch/blitzKrig/blob/main/vignettes/meuse_vignette_files/figure-gfm/universal_kriging-1.png)<!-- -->

``` r
# print the GLS estimates for coefficients of the linear predictor
pars_UK = fit_result_UK$pars
bk_GLS(g_meuse, pars_UK, X=X, out='b')
```

    ## [1]  5.681606  1.238569 -1.884427

These fitted kernels can be visualized using `bk_plot_pars`. This
plots the footprint of correlations around any given central point on
the grid.

``` r
# plot the kernel shape
bk_plot_pars(pars_UK, g_meuse)
```

![](https://github.com/deankoch/blitzKrig/blob/main/vignettes/meuse_vignette_files/figure-gfm/kernel_plot-1.png)<!-- -->
...
z_gls = bk_GLS(g_meuse, pars_UK, X=meuse_predictors, out='z')
g_meuse_gls = modifyList(g_meuse, list(gval=z_gls))
bk_plot(g_meuse_gls, main='estimated trend component')
```

![](https://github.com/deankoch/blitzKrig/blob/main/vignettes/meuse_vignette_files/figure-gfm/GLS_plot-1.png)<!-- -->

Once you have a set of covariance parameters, interpolation becomes very
easy with `bk_cmean`. This computes the kriging predictor, an
...
z_spat = bk_cmean(g_meuse_detrend, pars_UK, X=0)
g_meuse_spat = modifyList(g_meuse, list(gval=z_spat))
bk_plot(g_meuse_spat,
           main='estimated spatial component')
```

![](https://github.com/deankoch/blitzKrig/blob/main/vignettes/meuse_vignette_files/figure-gfm/spatial_plot-1.png)<!-- -->

``` r
# compute UK predictions, masking outside observed range
z_pred = z_gls + z_spat
g_meuse_pred = modifyList(g_meuse, list(gval=z_pred))
bk_plot(g_meuse_pred,
           zlim=log(range(meuse[['soils']][['zinc']])),
           zlab='log(zinc)',
           main='kriging predictor')
```

![](https://github.com/deankoch/blitzKrig/blob/main/vignettes/meuse_vignette_files/figure-gfm/predictor_plot-1.png)<!-- -->

Kriging variance is also computed with `bk_cmean`. This is much
...
![](https://github.com/deankoch/blitzKrig/blob/main/vignettes/meuse_vignette_files/figure-gfm/variance_plot-1.png)<!-- -->
...
bk_plot(g_meuse_pred2,
           zlab='zinc (ppm)',
           zlim=range(meuse[['soils']][['zinc']]),
           main='UK predictions (masked to input range)')
```

![](https://github.com/deankoch/blitzKrig/blob/main/vignettes/meuse_vignette_files/figure-gfm/predictor-1.png)<!-- -->

## Summary

This vignette is intended to get users up and running with `blitzKrig` by
demonstrating a very simple example on a familiar dataset. In other
vignettes we will look in more detail at how `bk_cmean` actually
...
if(FALSE)
{
  ...
  md.github = gsub('https://github.com/deankoch/blitzKrig/blob/main', 'https://github.com/deankoch/blitzKrig/blob/main', readLines(path.output))
